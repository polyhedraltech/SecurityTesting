...
/**
 * Menu item for toggling the ZAP allow attack on start ascan option.
 */
public class AllowAttackOnStartToggle extends AbstractMenuToggle {

	public AllowAttackOnStartToggle() {
		super("Allow Attack On Start");
	}
	...
	protected void executeToggleOff() {
		try {
			ZAPHelper.getInstance().getZAPClient().ascan
					.setOptionAllowAttackOnStart(ZAPHelper.getInstance().getZapApiKey(), false);
		} catch (ClientApiException e) {
			ConsolePlugin.log(e);
		}
	}
